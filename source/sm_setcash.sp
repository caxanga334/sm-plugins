...
public OnPluginStart()
{
	...
	CreateConVar("sm_setcash_version", PLUGIN_VERSION, "SetCash", FCVAR_DONTRECORD|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public Action:Command_SetCash(client, args)
{
	...
	for(new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, target_list[i], "\"%L\" set cash (%d) on \"%L\"", client, iCash, target_list[i]);
	}
	
	if (tn_is_ml)
	{
		ShowActivity2(client, "[SM] ", "Set cash (%d) on %t.",iCash, target_name);
	}
	else
	{
		ShowActivity2(client, "[SM] ", "Set cash (%d) on %s.",iCash, target_name);
	}
...
public Action:Command_AddCash(client, args)
{
	...
	for(new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, target_list[i], "\"%L\" added cash (%d) to \"%L\"", client, iCash, target_list[i]);
	}
	
	if (tn_is_ml)
	{
		ShowActivity2(client, "[SM] ", "Added cash (%d) to %t.",iCash, target_name);
	}
	else
	{
		ShowActivity2(client, "[SM] ", "Added cash (%d) to %s.",iCash, target_name);
	}
...
public Action:Command_RemoveCash(client, args)
{
	...
	{
		SetCash(target_list[i], GetCash(target_list[i])-iCash);
		LogAction(client, target_list[i], "\"%L\" removed cash (%d) from \"%L\"", client, iCash, target_list[i]);
	}
	
	if (tn_is_ml)
	{
		ShowActivity2(client, "[SM] ", "Removed cash (%d) from %t.",iCash, target_name);
	}
	else
	{
		ShowActivity2(client, "[SM] ", "Removed cash (%d) from %s.",iCash, target_name);
	}
